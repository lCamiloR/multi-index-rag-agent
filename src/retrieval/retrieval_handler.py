...
class RetrievalHandler(BaseEmbeedngHandler):
    """Load one FAISS index and expose a retriever-backed tool for LangGraph."""

    def __init__(
        self,
        embedding_model: str,
        faiss_local_path: str | Path,
        index_name: str = "index",
    ) -> None:
        super().__init__(embedding_model, faiss_local_path)
        self.loaded_vectorstore: FAISS = FAISS.load_local(
    ...
    def _load_retrieval(self, top_k: int) -> VectorStoreRetriever:
        """Create the retriever for the configured ``top_k``."""
        return self.loaded_vectorstore.as_retriever(
